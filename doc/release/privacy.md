...
| **Không network** | `pubspec.yaml` không có HTTP/Dio/socket client; AndroidManifest **không** `INTERNET` permission — app không thể gửi dữ liệu đi đâu cả |
| Permission duy nhất | `POST_NOTIFICATIONS` (Android 13+ runtime request) — duy nhất trong manifest |
...
| Analytics / crash reporting | **KHÔNG CÓ** — không dependency analytics/crash trong pubspec |
| Ads / trackers / SDK bên thứ ba thu data | KHÔNG CÓ |

## 1. Data collection (khai báo store — Data safety / Privacy nutrition)

| Câu hỏi store | Trả lời | Basis |
|---|---|---|
| Does the app collect or share personal data? | **No** | Không network permission, không analytics SDK |
| Data encrypted in transit? | N/A — không truyền dữ liệu | Không network |
...
> Lưu ý trung thực: tuyên bố "no data collected" **đúng** cho cấu hình release hiện tại vì app không có khả năng truyền dữ liệu (không network permission). Nếu sau này thêm bất kỳ SDK network/analytics → **phải** sửa file này + khai báo store trước khi ship.
...
1. Data we collect
   ShiftEase does not collect, transmit, or sell any personal data. The app
   has no internet permission, no analytics, and no advertising SDKs. There
   is no server.
