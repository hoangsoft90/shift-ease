...
- [ ] **P8 nhóm B (human/device — không agent tự PASS):** push → CI xanh · signed AAB + iOS archive · smoke/upgrade PASS thật · internal + closed testing · zero P0/P1 → chỉ khi đó P8 DONE → P9 *(phần device trùng với P9 nhóm B — làm chung một chu kỳ)*
- [x] **Phase design ↔ code audit (2026-09-11, `result_phase_design_audit.md`):** Overall **MOSTLY MATCH** — 0 P0/P1 drift; mọi invariant cốt (review-before-commit, atomic re-version, fail-closed SQLCipher, no systemTemp backup, UNAVAILABLE-not-partial, append-only override) = CODE MATCH với evidence path. Findings: F-001 ICS systemTemp residual (P3) · F-002 CI chưa có run GitHub nào (P2 process) · F-003 signing scaffold (P2 process). Doc/claim mismatches: **0 còn tồn tại**. Analyze clean · **315/315** · cipher_proof SQLCipher 4.18.0. Ưu tiên tiếp: commit tree → push → CI xanh → signing config.
- [ ] **P9 nhóm B (human/store — `result_p9_launch.md` §9):** push → CI xanh · keystore/certs → signed AAB/IPA · điền submission record (checksum + CI link) · host policy URL + support email · submit → staged rollout → monitoring · first production review → chỉ khi đó **PRODUCTION LIVE / P9 DONE**
- [x] **P9 LAUNCH PREP COMPLETE (2026-09-11, `result_p9_launch.md` — ops docs đủ, KHÔNG claim LIVE):** P9.1 final release gate (P0–P8 honest + waiver user ≠ PASS, 0 known ship-blocker, GATE OPEN với 4 điều kiện) · P9.2 submission record template (checksum + no-rebuild rule) · P9.3 staged launch (10%→100% + halt criteria) · P9.4/5 monitoring + triage (nguồn thật: store console/support/canary — không bịa analytics) · P9.6 hotfix procedure (minimal, no feature) · P9.7 data-loss incident (NEVER xóa DB first response) · P9.8 first production review template · P9.9 backlog list-only (9 items, không code) — suite **315/315**, analyze clean, không đổi app behavior
