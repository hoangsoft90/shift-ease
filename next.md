...
- [ ] **Audit độc lập (2026-09-11, `result_phase_design_audit.md`):** MOSTLY MATCH — 0 P0/P1 code drift; việc tiếp theo theo thứ tự audit đề xuất: (1) commit tree hiện tại, (2) push + CI xanh (F-002), (3) keystore + signingConfig (F-003), (4) optional ICS fallback fix (F-001), (5) device cycle dù waived.
- [ ] **P9 nhóm B — production launch (human/store — `result_p9_launch.md` §9):** push GitHub → CI xanh (điều kiện cuối P8.0) · keystore/certs → signed AAB/IPA (sửa signingConfig release trước — `doc/release/build_notes.md`) · điền `doc/release/submission_record.md` (SHA-256 + CI link) · host policy URL + support email · submit → staged rollout (`doc/release/staged_launch.md`) → monitoring (`doc/release/monitoring.md`) → first production review (7–14 ngày) → chỉ khi đó **PRODUCTION LIVE / P9 DONE**. **P9 LAUNCH PREP COMPLETE (2026-09-11):** toàn bộ ops docs P9.1–P9.9 trên disk trong `doc/release/` (gate, submission, staged, monitoring+triage, hotfix, data-loss, review, backlog) — KHÔNG claim LIVE; waiver device/signed/closed của user ≠ PASS
- [ ] **P8 nhóm B / P7 device legs (human/device — làm chung một chu kỳ với P9 nhóm B):** push GitHub → CI green · signed build · smoke + upgrade thật (`doc/release/`) · internal → closed testing (P0/P1 = stop ship) · negative test copy-DB→plain-SQLite · Keystore/Keychain kill/reboot · notification matrix · matrix `doc/mobile_readiness.md`. Nhóm A (prep) ✅ complete: freeze/version/checklists/store/privacy/protocols
